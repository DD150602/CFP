...
export function generarPdf (datos) {
  ...
  doc.setFontSize(16)
  doc.text(`Deudas en ${data.registros_bancarios[0].mes_registro}`, calculoCentral(`Deudas en ${data.registros_bancarios[0].mes_registro}`), 36)
  ...
  generarFinalDocumento()
  doc.setFont(undefined, 'bold')
  impresionTexto(`Este documento hace constancia del reporte del ${data.registros_bancarios[0].mes_registro}, el cual ha sido expedido el día ${dayjs().format('DD/MM/YYYY')}, para cualquier trámite o papeleo necesario del cliente.`, doc.internal.pageSize.getHeight() - 22)
